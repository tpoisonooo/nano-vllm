...
@triton.jit
def softmax_kernel_persistant(input, output, n_rows, n_cols, BLOCK_SIZE: tl.constexpr, num_stages: tl.constexpr, ROWS_PER_PROG: tl.constexpr):
    # starting row of the program
    pid = tl.program_id(0)
    start_row = pid * ROWS_PER_PROG

    for row in tl.range(start_row, start_row+ROWS_PER_PROG):
        if row < n_rows:
            input_ptr = input + row * n_cols
            output_ptr = output + row * n_cols
...
def persistant(x):
    ...
    # 使用 LaunchParam 计算超参
    # var_count=3: kernel 中有 cur_max, exp_sum, x 等变量
    param = LaunchParam()
    param.deduce(n_rows, n_cols, var_count=3, dev=str(DEVICE), target_occupancy=0.9)
    
    # Create a number of persistent programs.
    # grid_size = ceil(n_rows / rows_per_prog)
    grid_size = (n_rows + param.rows_per_prog - 1) // param.rows_per_prog
    
    softmax_kernel_persistant[(grid_size, 1, 1)](
        x, y, n_rows, n_cols, 
        BLOCK_SIZE=param.block_size, 
        num_stages=param.num_stages, 
        ROWS_PER_PROG=param.rows_per_prog,
        num_warps=param.num_warps
    )
    return y
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=[128 * i for i in range(2, 8192, 256)],  # different possible values for `x_name`
        line_arg='provider',  # argument name whose value corresponds to a different line in the plot
        line_vals=['tc', 'torch', 'v1', 'v2', 'persistant'],  # possible values for `line_arg``
        line_names=["tc", "torch", "v1", "v2", "persistant"],  # label name for the lines
        styles=[('blue', '-'), ('green', '-'), ('red', '-'), ('yellow', '-'), ('pink', '-')],  # line styles
...
def benchmark(M, N, provider):
    ...
    if provider == 'persistant':
        ms = triton.testing.do_bench(lambda: persistant(x))
    if provider == 'tc':
