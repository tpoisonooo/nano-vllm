...
class ThreadSafeLayerProfiler:
    ...
    def _post_hook(self, module, input, output):
        ...
        if module_id in self._start_times:
            ...
            name = getattr(
                module, "_layer_name", f"{type(module).__name__}_{module_id}"
            )
    ...
    def print_stats(self, top_k=-1, levels=[3, 4], skip="model.layers.0"):
        """打印统计结果"""
        print(f"\n{'=' * 80}")
        ...
        for stat in self.get_stats()[:top_k]:
            stat_name = stat["name"]
            if skip in stat_name:
                continue
            level_count = stat_name.count(".")
            if level_count not in levels:
                continue
            print(
